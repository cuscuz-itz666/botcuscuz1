...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Cuscuz\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=553183411248:+55 3183411248\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD CUSCUZ BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Cuscuz? Este é um recurso especial para o Cuscuz ❌',
					ownerB: '[❗] Cuscuz? Este é um recurso especial para o Cuscuz ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de CUSCUZ. Registre-se para fazer amizade com o bot Cuscuz por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Cuscuz|17\n\n──「 CUSCUZ-BOT 」──`,
			...
			const mod = [ownerNumber,"553183411248@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["553183411248@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["553183411248@s.whatsapp.net"]//ubah nomor lo
			const premium = ["553183411248@s.whatsapp.net","553191159967@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("pq vc e gay")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('cara, nao fale essas coisas, é errado, mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS CUSCUZ BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Cuscuz domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Cuscuz ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT CUSCUZ ✨* 」\n*│+ wa.me/553183411248*╰──────*「 *CUSCUZ* 」*────*\n\n*_SE QUER SER ADMIN DO BOT CUSCUZ*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:CUSCUZ*\n*WPP:* wa.me/+553183411248\n*INSTA:* instagram.com/cuscuz_com_queijo & instagram.com/bruno0campos\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
